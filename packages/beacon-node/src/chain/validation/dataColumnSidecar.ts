...
// used in network/processor/gossipHandlers.ts
export async function validateGossipDataColumnSidecar(
  ...
  // Todo: consider merging with kzgVerificationDataColumnBatchTime
  ...
  try {
    // Note: only one data column is being verified here
 ...
 */
// used in beacon-node/src/sync/utils/downloadByRange.ts and downloadByRoot.ts
export async function validateBlockDataColumnSidecars(
  ...
  chain?.metrics?.peerDas.dataColumnSidecarProcessingRequests.inc();
  ...
  try {
    ...
    for (let i = 0; i < dataColumnSidecars.length; i++) {
      ...
      const valid = verifyDataColumnSidecarInclusionProof(columnSidecar);
      timer?.();
      if (!valid) {
        throw new DataColumnSidecarValidationError(
          {
    ...
    // batch verification in both the cases: downloadByRange and downloadByRoot
